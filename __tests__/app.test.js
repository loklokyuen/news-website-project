		...
		describe("sort_by", () => {
			test("200: Responds with an array of all articles sorted by the specified column", () => {
				const sortByNumberValuesInsideString = (a, b) => {
					...
					else if (Number(a.cost_at_auction) < Number(b.cost_at_auction))
						return -1;
					else return 0;
				};
				return request(app)
					...
					.then(({ body: { articles } }) => {
						...
						expect(articles).toBeSortedBy("comment_count", {
							descending: true,
							compare: sortByNumberValuesInsideString,
						});
					});
			});
			...
			return request(app)
				...
				.then(({ body: { comments } }) => {
					...
					comments.forEach((comment) => {
						...
						expect(typeof comment.avatar_url).toBe("string");
	...
	describe("POST /api/articles", () => {
		test("200: Responds with the newly created article", () => {
			const articleToAdd = {
				...
				topic: "cats",
				article_img_url:
					"https://images.pexels.com/photos/45170/kittens-cat-cat-puppy-rush-45170.jpeg?w=700&h=700",
			};
			return request(app)
				.post("/api/articles")
				.send(articleToAdd)
